...
openssl ca -batch -in aws-server.csr -cert private-ca.crt -keyfile private-ca.key -out aws-server.crt
...
mk_client() {
  ...
  openssl ca -batch -in aws-client$1.csr -cert private-ca.crt -keyfile private-ca.key -out aws-client$1.crt
...
openssl ca -batch -gencrl -keyfile private-ca.key -cert private-ca.crt -out private-ca.crl1

# Revoke 1st client cert

openssl ca -batch -revoke aws-client1.crt -keyfile private-ca.key -cert private-ca.crt
openssl ca -batch -gencrl -keyfile private-ca.key -cert private-ca.crt -out private-ca.crl2
