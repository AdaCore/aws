...
procedure H2Post_Attachments is
...
--   Client.Create (WC, Server.Status.Local_URL (WS), HTTP_Version => HTTPv2);
   ...
   R := AWS.Client.Post
     (Server.Status.Local_URL (WS) & "/Upload", Data => "ID=100",
      HTTP_Version => HTTPv2);
   Text_IO.Put_Line ("R-POST1 : " & Response.Message_Body (R));
   Text_IO.New_Line;

   R := AWS.Client.Post
     (Server.Status.Local_URL (WS) & "/Upload", Data => "ID=101",
   ...
   R := AWS.Client.Post
     (Server.Status.Local_URL (WS) & "/Upload?ID=102",
   ...
   Text_IO.New_Line;
    ...
    R := AWS.Client.Post
      (Server.Status.Local_URL (WS) & "/Upload?ID=103", Data => "AnyOldString",
    ...
    R := AWS.Client.Post
      (Server.Status.Local_URL (WS) & "/Upload", Data => "ID=104",
       Content_Type => AWS.MIME.Application_Form_Data,
       Attachments  => Attachments,
       HTTP_Version => HTTPv2);
