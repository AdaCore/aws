...
procedure AttSimCont is
   ...
   procedure Call (ex : n1_ExampleType_Type);
   ...
   procedure Call (ex : n1_ExampleType_Type) is
   begin
      Text_IO.Put_Line ("ExampleType: " & ex'Image);
   end Call;

   H_Server : Server.HTTP;
   CNF      : Config.Object;
   P        : constant n1_Exampletype_Type := (1, To_Unbounded_String ("cm"));
...
begin
   ...
   Text_IO.Put_Line ("ex:");
   Text_IO.Put_Line
     (SOAP.Types.XML_Image (To_SOAP_Object (P, "ExampleType"),
                            Encoding => SOAP.WSDL.Schema.Literal,
                            Schema   => AttSimContService.Schema));

   AttSimContService.Client.Call (P);
