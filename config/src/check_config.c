#define CVS_ID "$Id$"
/*
   This program is used to get some OS specific settings
*/

#include <stdio.h>
#include <stdlib.h>

#ifdef _WIN32
#define WIN2000SUPPORT
/* We define WIN2000SUPPORT to ensure that executables built with this version
   will run ok on both Win2000 and WinXP.  */
#include <ws2tcpip.h>
#else /* ! _WIN32 */
#include <sys/types.h>
#include <sys/poll.h>
#include <sys/socket.h>
#include <sys/ioctl.h>
#include <netdb.h>
#include <netinet/in.h>
#include <netinet/tcp.h>
#include <errno.h>
#endif

#ifdef sun
#include <sys/filio.h>
#endif

/* File output routines to ensure UNIX line ending */

FILE *fd;

#define MAX_BUFFER 100

void
P (const char *str)
{
  char buf[MAX_BUFFER];
  char *p = buf;

  strcpy (buf, str);

  while (*p++) 
    {
      if (*p == '\n')
	*p = '\x0a';
    }
  fprintf (fd, buf);
}

void
PI (char *str, int val)
{
  char res[MAX_BUFFER];
  sprintf (res, str, val);
  P (res);
}

PS (char *str, const char *val)
{
  char res[MAX_BUFFER];
  sprintf (res, str, val);
  P (res);
}

/* For systems without IPv6 support */

#ifndef AI_PASSIVE
#define NO_IPV6_SUPPORT
#define AI_PASSIVE -1
#define AI_CANONNAME -1
#define AI_NUMERICHOST -1
#define EAI_SYSTEM -1
#define PF_INET6 -1
#define AF_INET6 -1
#endif

int
main (int argc, char *argv[])
{
#ifndef NO_IPV6_SUPPORT
  const struct addrinfo ai;

  const void *ai_ptr = &ai;

  const int ai_flags_offset     = (void *)&ai.ai_flags     - ai_ptr;
  const int ai_family_offset    = (void *)&ai.ai_family    - ai_ptr;
  const int ai_socktype_offset  = (void *)&ai.ai_socktype  - ai_ptr;
  const int ai_protocol_offset  = (void *)&ai.ai_protocol  - ai_ptr;
  const int ai_addrlen_offset   = (void *)&ai.ai_addrlen   - ai_ptr;
  const int ai_addr_offset      = (void *)&ai.ai_addr      - ai_ptr;
  const int ai_canonname_offset = (void *)&ai.ai_canonname - ai_ptr;
  const int ai_next_offset      = (void *)&ai.ai_next      - ai_ptr;
#else
  const int ai_flags_offset     = -8;
  const int ai_family_offset    = -7;
  const int ai_socktype_offset  = -6;
  const int ai_protocol_offset  = -5;
  const int ai_addrlen_offset   = -4;
  const int ai_addr_offset      = -3;
  const int ai_canonname_offset = -2;
  const int ai_next_offset      = -1;
#endif

#if defined(__FreeBSD__) || defined(_WIN32)
  const int s_nfds_t = sizeof (int) * 8;
#else
  const int s_nfds_t = sizeof (nfds_t) * 8;
#endif

#ifdef _WIN32
  #define ETIMEDOUT   WSAETIMEDOUT
  #define EWOULDBLOCK WSAEWOULDBLOCK
  #define EINPROGRESS WSAEINPROGRESS
  #define SHUT_RDWR   SD_BOTH
  #define EAI_SYSTEM  EAI_AGAIN
  // EAI_SYSTEM getaddrinfo error code does not supported by Win32, but need
  // for correct error handling, we could use any Win32 socket error code
  // instead, because Win32 use the same error codes in Errno report.

  const char *i_conv      = "Stdcall";
  const int s_fd_type     = sizeof (int) * 8;
  const int s_events_type = sizeof (short) * 8;
  const int v_POLLIN      = 1;
  const int v_POLLPRI     = 2;
  const int v_POLLOUT     = 4;
  const int v_POLLERR     = 8;
  const int v_POLLHUP     = 16;
  const int v_POLLNVAL    = 32;
#else
  const char *i_conv      = "C";
  const struct pollfd v_pollfd;
  const int s_fd_type     = sizeof (v_pollfd.fd) * 8;
  const int s_events_type = sizeof (v_pollfd.events) * 8;
  const int v_POLLIN      = POLLIN;
  const int v_POLLPRI     = POLLPRI;
  const int v_POLLOUT     = POLLOUT;
  const int v_POLLERR     = POLLERR;
  const int v_POLLHUP     = POLLHUP;
  const int v_POLLNVAL    = POLLNVAL;
#endif

  if (argc == 2)
    fd = fopen (argv[1], "wb");
  else
    {
      fprintf (stderr, "Syntax: check_config <filename>\n");
      exit (1);
    }

  P ("\n--  AUTOMATICALLY GENERATED, DO NOT EDIT THIS FILE\n\n");
  PS ("--  Generator: %s\n\n", CVS_ID);
  P ("with Interfaces.C.Strings;\n");
  P ("with System;\n\n");
  P ("package AWS.OS_Lib.Definitions is\n\n");
  P ("   use Interfaces;\n\n");

#ifdef _WIN32
  // libaws_win32.a need for emulate some unix routines not available in Win32.
  P ("   pragma Linker_Options (\"-laws_win32\");\n");
  // libws2_32.a need for getaddrinfo freeaddrinfo routines in Windows XP/2003.
  P ("   pragma Linker_Options (\"-lws2_32\");\n\n");
#endif

  /* POLL constants */

  PI ("   POLLIN   : constant := %d;\n", v_POLLIN);
  PI ("   POLLPRI  : constant := %d;\n", v_POLLPRI);
  PI ("   POLLOUT  : constant := %d;\n", v_POLLOUT);
  PI ("   POLLERR  : constant := %d;\n", v_POLLERR);
  PI ("   POLLHUP  : constant := %d;\n", v_POLLHUP);
  PI ("   POLLNVAL : constant := %d;\n\n", v_POLLNVAL);

  /* getaddrinfo constants */

  PI ("   AI_PASSIVE     : constant := %d;\n", AI_PASSIVE);
  PI ("   AI_CANONNAME   : constant := %d;\n", AI_CANONNAME);
  PI ("   AI_NUMERICHOST : constant := %d;\n", AI_NUMERICHOST);
  PI ("   EAI_SYSTEM     : constant := %d;\n\n", EAI_SYSTEM);

  /* other constants */

  PI ("   IPPROTO_TCP : constant := %d;\n", IPPROTO_TCP);
  PI ("   IPPROTO_IP  : constant := %d;\n", IPPROTO_IP);
  PI ("   PF_UNSPEC   : constant := %d;\n", PF_UNSPEC);
  PI ("   PF_INET     : constant := %d;\n", PF_INET);
  PI ("   PF_INET6    : constant := %d;\n", PF_INET6);
  PI ("   AF_INET     : constant := %d;\n", AF_INET);
  PI ("   AF_INET6    : constant := %d;\n", AF_INET6);
  PI ("   SO_ERROR    : constant := %d;\n", SO_ERROR);
  PI ("   SO_SNDBUF   : constant := %d;\n", SO_SNDBUF);
  PI ("   SO_RCVBUF   : constant := %d;\n", SO_RCVBUF);
  PI ("   TCP_NODELAY : constant := %d;\n", TCP_NODELAY);
  PI ("   SOCK_STREAM : constant := %d;\n", SOCK_STREAM);
  PI ("   SOL_SOCKET  : constant := %d;\n", SOL_SOCKET);
  PI ("   SHUT_RDWR   : constant := %d;\n", SHUT_RDWR);
  PI ("   ETIMEDOUT   : constant := %d;\n", ETIMEDOUT);
  PI ("   EWOULDBLOCK : constant := %d;\n", EWOULDBLOCK);
  PI ("   EINPROGRESS : constant := %d;\n", EINPROGRESS);
  PI ("   FIONBIO     : constant := %d;\n", FIONBIO);
  PI ("   FIONREAD    : constant := %d;\n\n", FIONREAD);

  /* nfds_t */

  PI ("   type nfds_t is mod 2 ** %d;\n", s_nfds_t);
  PI ("   for nfds_t'Size use %d;\n\n", s_nfds_t);

  /* FD_Type */

  PI ("   type FD_Type is mod 2 ** %d;\n", s_fd_type);
  PI ("   for FD_Type'Size use %d;\n\n", s_fd_type);

  /* Events_Type */

  PI ("   type Events_Type is mod 2 ** %d;\n", s_events_type);
  PI ("   for Events_Type'Size use %d;\n\n", s_events_type);

  /* Addr_Info */

  if (ai_flags_offset > 0
      || ai_family_offset    >= ai_socktype_offset
      || ai_socktype_offset  >= ai_protocol_offset
      || ai_addrlen_offset   >= ai_addr_offset
      || ai_addrlen_offset   >= ai_canonname_offset
      || ai_addr_offset      >= ai_next_offset
      || ai_canonname_offset >= ai_next_offset)
      //  Broke source code because of
      P ("   Unexpected addrinfo fields order.");

  P ("   type Addr_Info;\n");
  P ("   type Addr_Info_Access is access all Addr_Info;\n\n");

  P ("   type Addr_Info is record\n");
  P ("      ai_flags     : C.int;\n");
  P ("      ai_family    : C.int;\n");
  P ("      ai_socktype  : C.int;\n");
  P ("      ai_protocol  : C.int;\n");
  P ("      ai_addrlen   : C.size_t;\n");

  if (ai_canonname_offset < ai_addr_offset) {
    //  Linux fields order.

    P ("      ai_canonname : C.Strings.chars_ptr;\n");
    P ("      ai_addr      : System.Address;\n");
  } else {
    // Win32, FreeBSD, Solaris fields order.

    P ("      ai_addr      : System.Address;\n");
    P ("      ai_canonname : C.Strings.chars_ptr;\n");
  }

  P ("      ai_next      : Addr_Info_Access;\n");
  P ("   end record;\n");
  P ("   pragma Convention (C, Addr_Info);\n\n");

  P ("   function GetAddrInfo\n");
  P ("     (node    : in     C.Strings.chars_ptr;\n");
  P ("      service : in     C.Strings.chars_ptr;\n");
  P ("      hints   : in     Addr_Info;\n");
  P ("      res     : access Addr_Info_Access)\n");
  P ("      return C.int;\n\n");

  P ("   procedure FreeAddrInfo (res : in Addr_Info_Access);\n\n");

  P ("   function GAI_StrError (ecode : in C.int) return C.Strings.chars_ptr;\n\n");

  P ("   function Set_Sock_Opt\n");
  P ("     (S       : in C.int;\n");
  P ("      Level   : in C.int;\n");
  P ("      OptName : in C.int;\n");
  P ("      OptVal  : in System.Address;\n");
  P ("      OptLen  : in C.int) return C.int;\n\n");

  P ("private\n\n");

#ifdef WIN2000SUPPORT
  P ("   function WSPIAPI_init return Integer;\n");
  P ("   pragma Import (C, WSPIAPI_init, \"WSPIAPI_init\");\n");
  P ("   Dummy : constant Integer := WSPIAPI_init;\n\n");

  P ("   pragma Import (Stdcall, GetAddrInfo, \"WspiapiGetAddrInfo\");\n");
  P ("   pragma Import (Stdcall, FreeAddrInfo, \"WspiapiFreeAddrInfo\");\n");
#else
  PS ("   pragma Import (%s, GetAddrInfo, \"getaddrinfo\");\n", i_conv);
  PS ("   pragma Import (%s, FreeAddrInfo, \"freeaddrinfo\");\n", i_conv);
#endif

  // gai_strerror for Win32 inlined in WS2TCPIP.H and is not thread safe.
  // We are using simple replacement in win32/gai_strerror.c

#ifdef _WIN32
  P ("   pragma Import (C, GAI_StrError, \"AWS_gai_strerror\");\n\n");
#else
  P ("   pragma Import (C, GAI_StrError, \"gai_strerror\");\n\n");
#endif

  PS ("   pragma Import (%s, Set_Sock_Opt, \"setsockopt\");\n\n", i_conv);

  P ("end AWS.OS_Lib.Definitions;\n");
  fclose (fd);

  return 0;
}
