...
package body Table_Of_Strings_And_Static_Values_G is
  ...
  procedure Dispose is
     new Unchecked_Deallocation (Object => String_Type, Name => Access_String_Type);
...
--/ QUERIES:
  ...
  function Is_Present (Table: in Table_Type; Key: in String_Type) return Boolean is
    Access_String: Access_String_Type := Create (Key);
    Found: Boolean;
  begin
  ...
  function Value (Table: in Table_Type; Key: in String_Type) return Value_Type is
    Access_String: Access_String_Type := Create (Key);
    Value: Value_Type;
  begin
...
--/ LOCAL SUBPROGRAM:
  ...
  procedure Get_Min_Key (Table: in Table_Type;
    ...
    Access_String: Access_String_Type;
  begin
    Local_Package.Get_Min_Key (Local_Package.Table_Type(Table), Access_String);
    Copy (Key, Last, Access_String);
  exception
    when Local_Package.Empty_Structure_Error =>
      raise Empty_Structure_Error;
  ...
  procedure Get_Max_Key (Table: in Table_Type;
    ...
    Access_String: Access_String_Type;
  begin
    Local_Package.Get_Max_Key (Local_Package.Table_Type(Table), Access_String);
    Copy (Key, Last, Access_String);
  exception
    when Local_Package.Empty_Structure_Error =>
      raise Empty_Structure_Error;
  ...
  procedure Get_Greater_Or_Equal_Item (Table: in Table_Type;
    ...
    Access_String: Access_String_Type := Create (Key_In);
  begin
    Local_Package.Get_Greater_Or_Equal_Item (Local_Package.Table_Type(Table),
                                              Access_String,
                                              Value);
    Copy (Key_Out, Last, Access_String);
  exception
  ...
  function Less_Key (Table: in Table_Type;
    ...
    Access_String: Access_String_Type := Create (Key);
  begin
    Local_Package.Get_Less_Key (Local_Package.Table_Type(Table), Access_String);
    declare
      -- STR: constant String_type := ACCESS_STRING.all;
      Str: String_Type (1..Access_String.all'Length) := Access_String.all;
    begin
      Destroy (Access_String);
      return Str;
    end;
