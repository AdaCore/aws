...
generic
  ...
  type Key(<>) is private; -- required

  type Item is private; -- required

  with function Key_For (Node : Item) return Key; -- required

  with function "<" (Left : Key; Right : Key) return Boolean is <>; -- required
