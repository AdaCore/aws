...
--+ entries of type (String_Type, VALUE_TYPE), where VALUE_TYPE is specified by a
--+ generic parameter. Such a couple will also be called an item. The string
...
--/ QUERIES:
  ...
  function Is_Present (Table: in Table_Type; Key: in String_Type) return Boolean;
  --+ OVERVIEW:
  --+   Returns TRUE if and only if an ITEM with key KEY is in TABLE.

  function Value (Table: in Table_Type; Key: in String_Type) return Value_Type;
  ...
  function Less_Key (Table: in Table_Type; Key: in String_Type) return String_Type;
  --+ OVERVIEW:
  ...
  function Less_Or_Equal_Key (Table: in Table_Type; Key: in String_Type) return String_Type;
  --+ OVERVIEW:
  ...
  function Greater_Key (Table: in Table_Type; Key: in String_Type) return String_Type;
  --+ OVERVIEW:
  ...
  function Greater_Or_Equal_Key (Table: in Table_Type; Key: in String_Type) return String_Type;
  --+ OVERVIEW:
