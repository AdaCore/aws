                          ...
                          3.3.0 release / SOAP 2.0.0

Authors:
   Dmitriy Anisimkov
   Pascal Obry                                              May 9th, 2015
...
implementations.
  ...
  SOAP/WSDL. An important rework of the SOAP/WSDL support has been
  done on this new release. The user's types as described in the
  schema are now properly kept all down on the SOAP messages. This
  requires that a schema definition be passed to some calls as
  described below. Note that this is only if using a WSDL with schema.
  The schema is generated by wsdl2aws and is into the root generated
  package. Let's call it <ROOT> below:

        SOAP.Message.XML.Load_Payload (Payload)
     ->
        SOAP.Message.XML.Load_Payload (Payload, <ROOT>.Schema)

        SOAP.Dispatchers.Callback.Create (HTTP_CB'Access, SOAP_CB'Access);
     ->
        SOAP.Dispatchers.Callback.Create
           (HTTP_CB'Access, SOAP_CB'Access, <ROOT>.Schema);

        SOAP.Client.Call (...);
     ->
        SOAP.Client.Call (..., Schema => <ROOT>.Schema);

  It may also be required to pass the type name and corresponding
  name-space if using some SOAP types constructors. That is all
  SOAP.Types.R, SOAP.Types.I, etc can be passed the actual type name
  (if derived from a standard xsd type) and name-space, for example:
     ...
     ->
        SOAP.Types.I
          (12,
           Type_Name => "tns:MyIny",
           NS        => Name_Space.Create ("tns", "http://my.schema.com/ns");
